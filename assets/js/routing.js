...
export function analyzeTrajectory(parsedData, targetStationCode) {
    ...
    // --- DE VEILIGHEIDS-FALLBACK ---
    if (!trajectoryInfo && routeCodes.length > parsedData.routeCodes.length) {
        const start = routeCodes[0];
        const end = routeCodes[routeCodes.length - 1];
        console.warn(`[DEBUG] Voorspelling mislukt! De engine probeerde een route te trekken van ${start} naar ${end} via AMF, maar kon ${end} niet vinden in trajecten.json.`);
